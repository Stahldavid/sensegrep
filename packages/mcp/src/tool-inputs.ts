...
export const DuplicateToolArgsSchema = z.object({
  ...
  jevPanel: z.enum(["staged", "composite", "score", "noul"]).optional().describe("Jev panel: staged default, composite legacy, score/noul relevance ablations"),
  jevRanking: z.enum(["eligible", "legacy", "score", "rrf", "useful", "noul", "confidence-baseline"]).optional().describe("Jev ranking strategy (default eligible; ignored for evidence-only)"),
  jevCandidates: z.number().int().min(1).max(80).optional(),
  jevAspects: z.array(z.string().trim().min(1).max(240)).min(1).max(6).optional(),
  jevBlocks: z.boolean().optional(),
  jevBundles: z.boolean().optional(),
