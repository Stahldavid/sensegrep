...
describe("language autodetection", () => {
  it("detects supported languages from file extensions", async () => {
    ...
    await writeFile(path.join(TEST_DIR, "e.vue"), "<script setup>const x = 1</script>\n")
    ...
    try {
      ...
      expect(ids).toEqual(expect.arrayContaining(["typescript", "python", "javascript", "java", "vue"]))
      ...
      expect(detected.find((d) => d.language === "vue")?.fileCount).toBe(1)
    } finally {
