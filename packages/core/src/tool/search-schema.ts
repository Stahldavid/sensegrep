...
export const CommonSearchShape = {
  ...
  jev: z.enum(["off", "evidence", "rerank", "both"]).optional().describe("Jev source evaluation via OpenRouter: defaults to both with a dedicated Jev credential, otherwise off. Explicit off disables remote evaluation."),
  ...
  jevStages: z.array(z.enum(['rerank','evidence','recovery'])).min(1).max(3).optional().describe('Explicit Jev stages for ablations; requires Jev enabled'),
  jevVerifyAspects: z.boolean().optional().describe('Experimental three-way final requirement verification'),
  jevRecoveryDepth: z.number().int().min(1).max(3).optional().describe('Experimental resolved-call recovery depth; default 1'),
  jevBeamWidth: z.number().int().min(1).max(3).optional().describe('Maximum recovery frontier width; default 3'),
  ...
  jevCandidates: z.number().int().min(1).max(80).optional().describe("Maximum Jev candidate evaluations (default 32 implementation packages for context, 20 symbols for search)"),
  jevAspects: z.array(z.string().trim().min(1).max(240)).min(1).max(6).optional().describe("Explicit query requirements for evidence coverage"),
  jevBlocks: z.boolean().optional().describe("Experimental AST excerpts for oversized functions (requires Jev both)"),
  jevBundles: z.boolean().optional().describe("Experimental same-file literal constant context for Jev judgments"),
  jevTimeoutMs: z.number().int().min(100).max(60_000).optional().describe("Whole Jev stage deadline in milliseconds (default 30000 for package context, 8000 otherwise)"),
