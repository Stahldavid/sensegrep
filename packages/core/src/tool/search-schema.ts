...
export const CommonSearchShape = {
  ...
  hybridMode: z.enum(["adaptive", "parallel"]).optional().describe("Parallel (default) collects both signals; adaptive opts into lexical skipping"),
  rerank: z.boolean().default(false).describe("Apply deterministic second-stage reranking"),
  jev: z.enum(["off", "evidence", "rerank", "both"]).optional().describe("Optional OpenRouter Jev evaluation; sends candidate source to OpenRouter (default off)"),
  ...
  jevCandidates: z.number().int().min(1).max(40).optional().describe("Maximum Jev candidate evaluations (default 20)"),
  jevTimeoutMs: z.number().int().min(100).max(60_000).optional().describe("Whole Jev stage deadline in milliseconds (default 8000)"),
...
export const SenseGrepParametersSchema = z.object({
  ...
  maxPerFile: z.number().int().nonnegative().optional().describe("Strict maximum results per file; default admits one primary and one complementary symbol (exact: 2)"),
